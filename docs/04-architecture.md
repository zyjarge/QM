...
| 组件 | 数量 | 说明 | 参考内存上限 |
|------|------|------|------------|
| qm-server | 1~2 | Java Spring Boot 模块化单体（P0 = 1 实例） | 512M |
| qm-web | 1 | React SPA 静态文件 + Nginx | 32M |
| qm-job | 1 | MQ 异步消费者（与 server 同镜像） | 256M |

### 5.2 中间件（官方镜像，零构建）

| 组件 | 镜像 | 端口 | 说明 |
|------|------|------|------|
| postgres | pgvector/pgvector:pg16 | 5432 | 主库 + pgvector |
| redis | redis:7-alpine | 6379 | 缓存/幂等键/分布式锁 |
| rabbitmq | rabbitmq:3.13-management-alpine | 5672 / 15672 | 事件总线 / 管理台 |
| meilisearch | getmeili/meilisearch:v1.8 | 7700 | 全文搜索 |
| minio | minio/minio:latest | 9000 / 9001 | 附件 / 归档导出 |

### 5.3 基础设施

- Nginx: HTTPS 反代（Let's Encrypt 免费证书自动续期）
- Docker Compose 编排（P0）→ K8s（预留）
- 每日 pg_dump 备份 + MinIO 生命周期管理

### 5.4 外部依赖

| 外部系统 | 用途 |
|---------|------|
| 飞书 OpenAPI | 群管理、消息、卡片、免登、通讯录（核心依赖） |
| 企业微信 API（P1） | 降级模式接入 |
| LLM API（P1） | AI 澄清助手 |
| Git / CI | 追溯关联 |
| 域名 DNS + 公网 IP | 飞书回调必需 |
...
│                                                       │
│   ┌─────────────────────────────────────────────┐    │
│   │  Docker Host（内部网络 qm-net）                │    │
│   │                                              │    │
│   │  应用层:                                       │    │
│   │  ┌──────────┐  ┌──────────┐  ┌──────────┐  │    │
│   │  │ qm-server │  │ qm-web   │  │ qm-job   │  │    │
│   │  │ (Java 17) │  │ (nginx)  │  │ (worker) │  │    │
│   │  │ :8080     │  │ :80(s)   │  │ (异步)   │  │    │
│   │  └─────┬────┘  └────┬─────┘  └────┬─────┘  │    │
│   │        │              │              │        │    │
│   │  中间件层:                                   │    │
...
│   │  Nginx (443)                 │                    │
│   │  → qm.yourdomain.com        │                    │
│   │  → /webhook/feishu/events    │    飞书回调入口    │
│   │  → /webhook/feishu/card     │    卡片按钮回调     │
│   │  Let's Encrypt 自动续期      │                    │
│   └──────────────────────────────┘                    │
└─────────────────────────────────────────────────────┘

总计 9~10 个容器
外网入口: 仅 nginx:443(HTTPS); IM 回调路径 /webhook/** Nginx 层独立限流
```
...
→ 2C4G 云机足够（预算留余量）
→ 2C8G 更宽松，可开 2 个 qm-server 实例做滚动更新
